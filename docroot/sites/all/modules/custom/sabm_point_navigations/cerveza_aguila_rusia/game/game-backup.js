...
ig.module("game.entities.coin").requires("impact.entity").defines(function() {
    EntityCoin = ig.Entity.extend({
        ...
        check: function(b) {
            !0 != ig.game.gamePause && b instanceof EntityPlayer && (ig.game.coinCollected++, ig.soundHandler.sfxPlayer.play("coin"), ig.game.saveCoinStats(), this.checkAgainst =
...
ig.module("game.entities.endcontrol").requires("impact.entity").defines(function() {
    EntityEndcontrol = ig.Entity.extend({
        ...
        init: function(b, c, d) {
            ...
            var coins = ig.game.coinCollected;
            this.saveResult(this.score, coins);
        },
        saveResult: function(score, coins) {
            var __p = '/user/' + (window.location !== window.parent.location ? parent._p : _p);
            $.post(__p, {points:score, coins: coins}, function(data, status) {
